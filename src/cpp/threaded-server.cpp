...
#include <array>
#include <iostream>
#include <sstream> // can be optional if thread id does not need to be printed
#include <thread>
#include <unistd.h>
...
main(int argc, char* argv[])
{
	...
	while (true) {
		...
		try {
			std::thread th(serve_connection, new_sockfd);
			std::cout << "thread " << th.get_id()
					  << " created to handle connection with socket "
					  << new_sockfd << std::endl;
			// Detach the thread - when it's done, its resources will be cleaned
			// up. Since the main thread lives forever, it will outlive the
			// serving threads.
			th.detach();
