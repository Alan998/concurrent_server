// build with go build threadpool.go threadpool-server.go
package main
...
func handle_connection(server_name string, max_threads int) {
	...
	// start threadpool
	pool := NewThreadPool(max_threads)
	pool.Run()
	...
	for {
		...
		pool.AddTask(func () {serve_connection(conn)})
	}
}
...
func main() {
	...
	max_threads := flag.Int("thread", 5, "number of worker threads in threadpool")
	flag.Parse()

	if *max_threads <= 0 {
		fmt.Println("Invalid argument: max_threads must be greater than zero")
		return
	}
	...
	handle_connection(server_name, *max_threads)
	return
}
